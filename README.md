...
|  💳  [Payment Subscriptions](https://github.com/CaioSNery/Payment-DDD) <br>  ![.NET](https://img.shields.io/badge/.NET-8.0-blue) | API desenvolvida em .NET 8 para gerenciamento de assinaturas de alunos, permitindo criar uma inscrição a partir de diferentes tipos de pagamento: Boleto - PayPal - Cartão de Crédito | ASP.NET Core 8Entity Framework Core, SQL Server (padrão, mas pode ser alterado para InMemory) ,Swagger para documentação ,Flunt (validações / notificações) |
